👋 Hi, I'm Dabasish Roy

🎓 Student | 💻 Aspiring Software Developer | 🚀 Tech Enthusiast

🚀 About Me
🔭 I’m currently working on Java & JavaFX Projects
🌱 I’m currently learning Data Structures & Algorithms
💡 Interested in Software Development & Problem Solving
🎯 Goal: To become a Skilled Software Engineer
⚡ Fun fact: I love building real-life problem solving projects
🛠️ Skills & Technologies
💻 Languages: Java, C++, JavaScript
🧩 Frameworks: JavaFX, React (learning)
🗄️ Database: MySQL, Firebase
⚙️ Tools: Git, GitHub, VS Code, IntelliJ IDEA
📂 Projects
📚 Library Management System
Built using JavaFX + Maven
Features: Book management, user system, issue/return tracking
🔐 Personal Data Leak Checker
Detects if user data is exposed in breaches
Includes real-time alerts & secure dashboard
📊 GitHub Stats

🔥 Streak Stats

🌐 Connect With Me
📧 Email: your-email@example.com
💼 LinkedIn: https://linkedin.com/in/your-profile
🌍 Portfolio: Coming Soon...
🧠 Currently Learning
📌 Advanced Java
📌 Data Structures & Algorithms
📌 Software Engineering Concepts
🎯 Future Goals
Build real-world impactful projects
Contribute to open-source
Get a job in top tech company
